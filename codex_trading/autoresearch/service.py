...
from .client import CodexResponsesClient
...
class ResearchConfig:
    ...
    output_dir: str = str(LAYOUT.autoresearch_runs_root)
...
def ensure_output_dir(output_root: str, run_label: Optional[str] = None) -> Path:
    root = Path(output_root).expanduser()
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    suffix = "_%s" % str(run_label).upper() if run_label else ""
    run_dir = root / ("%s%s" % (timestamp, suffix))
...
def prepare_data_bundle(
    ...
    if data_bundle is not None:
        ...
        cache_root = LAYOUT.autoresearch_cache_root
...
""" % (spec_json, winner_spec.benchmark_symbol)
...
def run_autoresearch(
    ...
    start_ts = time.time()
    run_dir = ensure_output_dir(config.output_dir, run_label=config.symbol)
