...
def load_feishu_bot_config(
    ...
    *,
    ...
    run_dir = Path(run_dir_text).expanduser().resolve() if run_dir_text else find_latest_run_dir(str(SCRIPT_DIR / "output"))
    notifications = NotificationConfig(
...
def _safe_send_notification(
    ...
    *,
    ...
    try:
        ...
        messenger.send_group_card(config.feishu.push_chat_id, title, body, body)
        return True
    except Exception as exc:
